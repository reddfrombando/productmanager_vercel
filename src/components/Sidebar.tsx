...
export const Sidebar: React.FC = () => {
  ...
  const { user, progress, streak } = usePlatform();
  ...
  const menuItems = [
    ...
    { name: "Contributor Portal", href: "/contribute", icon: PlusCircle },
  ];
      ...
      {user && (
        <div className="p-4 border-b border-border-light bg-[#F8FAFC]/50">
          <div className="flex items-center space-x-3 rounded-xl bg-gradient-to-r from-orange-500/10 to-amber-500/10 p-3 border border-orange-500/10">
            ...
            <div>
              <div className="text-xs font-semibold text-primary/60">Study Streak</div>
      ...
      <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
            ...
            >
              <Icon
                className={`h-4.5 w-4.5 transition-transform group-hover:scale-110 ${
                  active ? "text-accent-purple" : "text-primary/50 group-hover:text-primary"
                }`}
              />
              <span>{item.name}</span>
            </Link>
        ...
        {user?.role === "admin" && (
          <div className="pt-4 border-t border-border-light/80 mt-4 space-y-1.5">
            ...
            >
              <Shield className={`h-4.5 w-4.5 ${isActive("/admin") ? "text-white" : "text-primary/50"}`} />
      ...
      {user && (
        <div className="p-4 border-t border-border-light bg-white flex items-center space-x-3">
          ...
          <div className="ml-auto">
            <Settings className="h-4 w-4 text-primary/40 hover:text-primary cursor-pointer transition-colors" />
          </div>
        </div>
      )}
    </aside>
